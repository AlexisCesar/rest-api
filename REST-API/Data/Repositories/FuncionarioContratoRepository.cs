...
namespace Restful_API.Data
{
    ...
    public class FuncionarioContratoRepository : IFuncionarioContratoRepository
    {
        ...
        {
            return await _context.ContratosCLT.Include(nameof(Contrato.Funcionario)).AsNoTracking().Where(x => x.FuncionarioId == funcionarioId).ToListAsync();
        }

        public async Task<IEnumerable<ContratoPJ>> GetContratosPJAsync(Guid funcionarioId)
        {
            return await _context.ContratosPJ.Include(nameof(Contrato.Funcionario)).AsNoTracking().Where(x => x.FuncionarioId == funcionarioId).ToListAsync();
        }
    }
}